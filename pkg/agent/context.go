...
Each part separated by the marker will be sent as an independent message.`,
	...
	for i, content := range extra {
		add(PromptPart{
			ID:      fmt.Sprintf("context.inline_extra.%d", i),
			Layer:   PromptLayerContext,
			Slot:    PromptSlotRuntime,
			Source:  PromptSource{ID: PromptSourceRuntime, Name: "inline_extra"},
			Title:   "inline system content",
			Content: content,
			Stable:  false,
		})
	}
